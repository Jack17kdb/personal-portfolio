...
const AskMe = () => {
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: "Hi! I am an AI assistant trained on Johnson's portfolio. Ask me anything about his projects, skills, background, or experience!",
    },
  ]);
  ...
  const hasUserStarted = messages.some((m) => m.role === "user");
  ...
  return (
    <section className="h-screen w-screen flex flex-col px-4 pt-20 pb-4">
      <div className="flex-1 overflow-y-auto rounded-xl border border-white/10 bg-white/[0.03] backdrop-blur-sm p-4 space-y-4 mb-4">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {SUGGESTIONS.map((s, i) => (
              <button
                ...
                disabled={loading}
                className="text-sm px-4 py-2 rounded-full border border-blue-500/30 bg-blue-500/10 text-blue-300 hover:bg-blue-500/20 hover:border-blue-400 transition-all duration-200 disabled:opacity-40"
              >
                {s}
              </button>
            ))}
        ...
        {messages.map((msg, idx) => (
          ...
          >
            ...
            <div
              className={
                "max-w-[85%] rounded-2xl px-4 py-3 text-base leading-relaxed whitespace-pre-wrap " +
                (msg.role === "user"
                  ? "bg-gradient-to-br from-blue-600/30 to-cyan-600/20 border border-blue-500/30 text-white rounded-tr-sm"
                  : msg.isError
        ...
        {loading && (
          <div className="flex gap-3">
            <div className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-gradient-to-br from-purple-600 to-pink-600 text-sm">
              <FiCpu />
            </div>
